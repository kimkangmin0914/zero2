...
# ------------------------------
# 핵심 솔버 (6인 고정, 교회≤2 기본/필요시 3·4, 나이대≤2 기본/필요시 3, 성비 2~4 슬랙)
# ------------------------------
...
# ------------------------------
# 사이드바 (고유 key로 DuplicateWidgetID 방지)
# ------------------------------
with st.sidebar:
    ...
    # 추가 기능 2: 글자 크기 조정 (최소값으로 너무 작아짐 방지)
...
if st.session_state.get("assignment_ready", False):
    ...
    with col1:
        if st.button("◀ 이전 팀", key="prev_team"):
            ...
            st.session_state.final_view = False
    with col2:
        st.markdown(f"<div class='team-title'>{st.session_state.team_idx+1} / {st.session_state.team_count}팀</div>", unsafe_allow_html=True)
    with col3:
    ...
    if st.session_state.final_view:
        ...
        for idx, line in enumerate(st.session_state.names_per_team, start=1):
            st.markdown(f"<div class='team-title'>팀 {idx}</div>", unsafe_allow_html=True)
            st.markdown(f"<div class='names-line'>{line}</div>", unsafe_allow_html=True)
    else:
        idx = st.session_state.team_idx
        st.markdown(f"<div class='team-title'>팀 {idx+1}</div>", unsafe_allow_html=True)
